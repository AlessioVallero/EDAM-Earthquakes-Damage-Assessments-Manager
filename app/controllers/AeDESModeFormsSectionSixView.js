...
function OnBtnBack_Click( e )
{
    try
    {
        ...
        {
            $.aedesModeSectionSixWindow.close() ;
        }
    }
    catch( exception )
    {
...
// Table view external danger click event handler
function OnTableViewAeDESModeFormsSectionSixExternalDanger_Click( e )
{
    try
    {
        Alloy.Globals.createAndOpenControllerExt( 'AeDESModeFormsSectionSixExternalDangerPotentialCause' , { external_danger_id: e.index , father_title: e.row.children[0].text , is_synchronized: current_is_synchronized } ) ;
    }
    catch( exception )
    {
        Alloy.Globals.AlertUserAndLogAsync( L( 'generic_exception_msg' ) + exception.message ) ;
