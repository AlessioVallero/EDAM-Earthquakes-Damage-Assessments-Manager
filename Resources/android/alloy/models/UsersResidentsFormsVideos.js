...
exports.definition = {
    config: {
        ...
        adapter: {
            type: "sql",
            collection_name: "UsersResidentsFormsVideos",
            db_name: "EEM",
            idAttribute: "ID"
        }
    },
...
model = Alloy.M("UsersResidentsFormsVideos", exports.definition, [ function(migration) {
    migration.name = "UsersResidentsFormsVideos";
    ...
    migration.down = function(migrator) {
        ...
        db.execute("CREATE TEMPORARY TABLE UsersResidentsFormsVideos_backup( ID , FORM_ID , VIDEO_PATH , LATITUDE , LONGITUDE , ADDRESS , DAMAGES_LEVEL , DAMAGES_AREA , COMMENT );");
        db.execute("INSERT INTO UsersResidentsFormsVideos_backup SELECT ID , FORM_ID , VIDEO_PATH , LATITUDE , LONGITUDE , ADDRESS , DAMAGES_LEVEL , DAMAGES_AREA , COMMENT FROM " + table + ";");
        migrator.dropTable();
        migrator.createTable({
            columns: {
                ID: "integer PRIMARY KEY AUTOINCREMENT",
        ...
        db.execute("INSERT INTO " + table + " SELECT ID , FORM_ID , VIDEO_PATH , LATITUDE , LONGITUDE , ADDRESS , DAMAGES_LEVEL , DAMAGES_AREA , COMMENT FROM UsersResidentsFormsVideos_backup;");
        db.execute("DROP TABLE UsersResidentsFormsVideos_backup;");
    };
} ]);

collection = Alloy.C("UsersResidentsFormsVideos", exports.definition, model);
